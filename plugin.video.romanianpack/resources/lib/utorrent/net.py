...
class UTorrent:
    ...
    def addnew_url(self, torrent, dirname, ids, id):
        self.add_url(torrent, dirname)
    
    def set_wanted(self, id, ind):
        return None
...
class Transmission:
    ...
    def list(self):
        obj = self.action({'method': 'torrent-get', 'arguments': {
            'fields': ['id', 'status', 'name', 'totalSize', 'sizeWhenDone', 'leftUntilDone', 'downloadedEver',
                       ...
                       'peersSendingToUs', 'wanted', 'hashString']}})
        if obj is None:
            return False
    ...
    def listwanted(self, id):
        obj = self.action({"method": "torrent-get", "arguments": {
            "fields": ["id", "desiredAvailable", "fileStats", "files", "wanted", "hashString"],
            "ids": [int(id)]}})['arguments']['torrents'][0]
        if obj is None:
            return None
    ...
    def addnew_url(self, torrent, dirname, ids, id):
        if self.action({'method': 'torrent-add', 'arguments': {'download-dir': dirname, 'filename': torrent, 'files-unwanted': ids, 'files-wanted' : id}}) is None:
            return None
        return True
    ...
    def set_wanted(self, id, ind):
        ...
        res = None
        inds = [int(ind)]
        res2 = self.action({"method": "torrent-verify", "arguments": {"ids": [int(id)]}})
        res = self.action({"method": "torrent-set", "arguments": {"ids": [int(id)], "files-wanted": inds}})

        return True if res or res2 else None
...
class Download():
    ...
    def addnew_url(self, torrent, dirname, ids, id):
        return self.client.addnew_url(torrent, dirname, ids, id)
            
    def set_wanted(self, id, ind):
        return self.client.set_wanted(id, ind)
