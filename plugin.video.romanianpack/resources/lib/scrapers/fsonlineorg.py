...
class fsonlineorg:
    ...
    def parse_menu(self, url, meniu, info={}):
        ...
        if meniu == 'recente' or meniu == 'cauta':
            ...
            s = requests.Session()
            second = "%s/wp-content/themes/grifus/loop/second.php" % base_url
            third = '%s/wp-content/themes/grifus/includes/single/second.php' % base_url
            reg_id = '''id[\:\s]+(\d+)[,\}]'''
            ...
            if re.search('/episodul/', url):
                ...
                html = ''
                while (j <= 4):
                    html += s.get('%s/wp-content/themes/grifus/loop/second_id.php?id=%s&embed=%s' % (base_url, mid, j), headers=headers).content
                    j = j + 1
                g = html
            else:
                f = s.post(third, data=data1, headers=headers)
                g = s.post(third, data=dataid, headers=headers).content
            ...
            if not re.search('/episodul/', url):
                ...
                try:
                    for links in match_lnk:
                        link = s.get('%s/%s' % (base_url,links), headers=headers).text
