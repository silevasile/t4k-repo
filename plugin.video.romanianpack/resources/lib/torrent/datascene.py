...
class datascene:
    ...
    categorii = [('Anime/Cartoons', '&cat=3'),
             ...
             ('TV/Pack', '&cat=54'),
             ('XXX', '&cat=12')]
    menu = [('Recente', "https://%s/browse.php" % base_url, 'recente', thumb)]
    menu.extend([(x[0], 'https://%s/browse.php?search=&blah=0%s&incldead=1' % (base_url, x[1]), 'sortare', thumb) for x in categorii])
    ...
    def login(self):
        ...
        headers = [('Host', base_url),
        ('User-Agent', 'Mozilla/5.0 (Windows NT 6.1; rv:70.1) Gecko/20100101 Firefox/70.1'),
        ('Referer', 'https://' + base_url + '/login.php'), ('X-Requested-With', 'XMLHttpRequest'), ('Accept', 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8'), ('Content-Type', 'application/x-www-form-urlencoded'), ('Accept-Language', 'ro,en-US;q=0.7,en;q=0.3')]
        x = self.makeRequest('https://%s/takelogin.php' % (self.base_url), data=data, headers=headers)
