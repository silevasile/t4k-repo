    ...
    def parse_menu(self, url, meniu, info={}, torraction=None):
        ...
        if meniu == 'get_torrent' or meniu == 'cauta' or meniu == 'recente':
            if meniu == 'cauta':
                ...
                regex_tr = '''(?:.+?torrents?.+?(cat=\d+)")?.+?(?:.+?title="(.+?ver.+?)")?(?:.+?;'>(.+?)<)?(?:.+?title="(.+?)")?(?:.+?genre.+?\:.+?>(.+?)<)?.+?href="(.+?)".+?<td.+?>(.+?)</td.+?<td.+?>(.+?)</td.+?<td.+?>(.+?)</td.+?>(.+?)</td.+?>(.+?)</td'''
                if None != response and 0 < len(response):
                    ...
                    for block in re.compile(regex, re.DOTALL | re.IGNORECASE).findall(response):
                        ...
                        if result:
                            for cat, verificat, nume, gold, genre, legatura, comments, added, size, seeds, leechers in result:
                                for r,t in self.categorii:
                                    if cat == t or not cat:
                                        size = striphtml(size)
                                        seeds = striphtml(seeds)
                                        leechers = striphtml(leechers)
                                        recomandat = ' Recomandat' if re.search('(recomand.+?recomand)', block, re.IGNORECASE) else ''
                                        free = '[COLOR lime]FreeLeech[/COLOR] ' if re.search('(freeleech\.png)', block) else ''
                                        sticky = ' Sticky' if re.search('(sticky\.png)', block) else ''
                                        rosubbed = ' ROSubbed' if re.search('(rosubbed\.png)', block) else ''
                                        rodubbed = ' Dublat' if re.search('(rodubbed\.png)', block) else ''
                                        added = ' %s' % added if added else ''
                                        gold = 'AUR' if gold else ''
                                        verificat = ' Verificat' if verificat else ''
